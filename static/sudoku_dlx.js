...
function solve_sudoku(grid, xSize, ySize)
{
    console.log("Solving at " + xSize + "x" + ySize);
    ...
    for (var i = 0; i < size; i++) {
        for (var j = 0; j < size; j++) {
            var g = grid[i][j] - 1;
            //var g = grid[size*i + j] - 1;
            if (g >= 0) {
                var row = new Array(cells*4);
                row[i*size+j] = 1;
                row[cells+i*size+g] = 1;
                row[cells*2+j*size+g] = 1;
                row[cells*3+(Math.floor(i/xSize)*xSize+Math.floor(j/ySize))*size+g] = 1;
                mat.push(row);
                ...
                for (var n = 0; n < size; n++) {
                    var row = new Array(cells*4);
                    row[i*size+j] = 1;
                    row[cells+i*size+n] = 1;
                    row[cells*2+j*size+n] = 1;
                    row[cells*3+(Math.floor(i/3)*3+Math.floor(j/3))*size+n] = 1;
                    mat.push(row);
